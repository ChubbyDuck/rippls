...
const printCommandOf = (call: unknown) => {
  ...
  return agent.buildPrintCommand({ prompt: 'x', dangerouslySkipPermissions: false }).command;
};
...
test('the same demand resolves against the agent that is running', async () => {
  const { Cursor } = registeredAgentsOf(repositoryRoot, path);
