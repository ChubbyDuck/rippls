...
import { TicketRepositoryLive } from './random';

const query = TicketQuery.build();

test('TicketRepositoryLive assigns a new id to each created ticket', () => {
  const ids = Effect.runSync(
    Effect.forEach(Array.from({ length: 100 }), () =>
      ...
      Effect.provide(TicketRepositoryLive),
      Random.withSeed('ids')
    )
  );
...
test('TicketRepositoryLive produces all ticket kinds equally', () => {
