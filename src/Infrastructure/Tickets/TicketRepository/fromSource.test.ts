...
test('ticketRepositoryLive reads from the folder source', async () => {
  const result = await Effect.runPromise(
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const dir = yield* fs.makeTempDirectory({ prefix: 'harness-source-' });
      yield* fs.writeFileString(
        path.join(dir, '1.md'),
        '---\nid: 1\ntitle: "From folder"\nproject: "demo"\nkind: "implementation"\nstatus: "ready-for-agent"\nblockedBy: []\nblocks: []\n---\n'
      );
      const resolved = yield* folderSource(dir);
      const tickets = yield* TicketRepository.pipe(
        Effect.flatMap((repo) => repo.getManyBy(TicketQuery.build())),
        Effect.provide(ticketRepositoryLive(resolved))
      );
...
test('ticketRepositoryLive reads from the linear source', async () => {
  ...
  const result = await Effect.runPromise(
    Effect.gen(function* () {
      ...
      const tickets = yield* TicketRepository.pipe(
        Effect.flatMap((repo) => repo.getManyBy(TicketQuery.build())),
        Effect.provide(ticketRepositoryLive(resolved).pipe(Layer.provide(api)))
      );
      return tickets.map((ticket) => ticket.title);
    })
  );
