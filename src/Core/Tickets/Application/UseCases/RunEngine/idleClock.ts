...
import { defaultIdleTimeout } from '~/Core/Shared/Domain/EngineConfig';
import { EngineIdleTimeout } from '~/Core/Tickets/Domain/Exceptions/EngineIdleTimeout';
...
export type IdleClock = {
  ...
  readonly awaitTimeout: Effect.Effect<never, EngineIdleTimeout>;
};
...
export const makeIdleClock = (idleTimeout: Duration.Input = defaultIdleTimeout): Effect.Effect<IdleClock> =>
  Effect.gen(function* () {
    ...
    const halted = yield* Deferred.make<never, EngineIdleTimeout>();
    ...
    const startTimeout = Effect.gen(function* () {
      ...
      const fiber = yield* Effect.sleep(idleTimeout).pipe(
        Effect.andThen(Deferred.fail(halted, new EngineIdleTimeout())),
        Effect.forkChild
      );
